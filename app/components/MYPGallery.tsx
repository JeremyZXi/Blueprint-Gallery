...
import { fetchApprovedMYPSubmissions, formatSubmissionsForGallery, fetchColorTags, fetchMaterialTags, type ColorTag, type MaterialTag } from "../utils/fetchSupabase";
...
const MYPGallery = () => {
  ...
  useEffect(() => {
    async function loadApprovedIAs() {
      setLoading(true);
      try {
        const submissions = await fetchApprovedMYPSubmissions();
        console.log('Fetched MYP submissions:', submissions);
        
        const formattedSubmissions = formatSubmissionsForGallery(submissions);
        console.log('Formatted MYP submissions:', formattedSubmissions);
        
        if (formattedSubmissions.length === 0) {
          console.log('No MYP submissions found, gallery will be empty');
          setIAs([]);
        } else {
          setIAs(formattedSubmissions);
        }
      } catch (error) {
        console.error("Failed to load MYP gallery items:", error);
        setIAs([]);
      } finally {
        setLoading(false);
      }
      ...
      <div className="container mx-auto">
        ...
        <div className="mb-12">
          <div className="flex flex-col md:flex-row justify-between mb-6">
            <h2 className="text-3xl font-serif mb-4 md:mb-0">MYP Design Projects</h2>
